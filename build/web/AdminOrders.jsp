<%-- 
    ...
    Created on : Apr 13, 2019, 10:09:22 PM
    Author     : user
--%>
<%
...
<nav>
    <a  href="AdminDashboard.jsp">Home</a>
            ...
            <a class="active" href="ShowOrders">Show Orders</a>
...
<table>
    <%
       int i=0;
       ArrayList<OrdersTable> ri = (ArrayList<OrdersTable>)session.getAttribute("orderdelivers");
       if(ri.size()!=0){
    %>
  <caption>View Orders Pending</caption>
  <thead>
    <tr>
  ...
  <tbody>
    ...
    <tr>
      ...
      %>
      <td data-label="Action"><a href="acceptOrder?us_name=<%= ri.get(i).getUname() %>&breed=<%= ri.get(i).getBreed() %>&gender=<%= ri.get(i).getGender() %>&age=<%= ri.get(i).getAge() %>" style="font-size:20px ;height:50px;width:100px;padding:10px 20px;">Accept</a><hr><a href="cancelOrder?us_name=<%= ri.get(i).getUname() %>&breed=<%= ri.get(i).getBreed() %>&gender=<%= ri.get(i).getGender() %>&age=<%= ri.get(i).getAge() %>" style="font-size:20px ;height:50px;width:100px;padding:10px 20px;background-color:red;color:white">Delete</a></td>
      <%
          }else{
      %>
      <td data-label="Action">DONE</td>
      <%
          }
      %>
    </tr>
